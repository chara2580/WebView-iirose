(function iiroseEmojiCategoriesScript() {
  ...
  function installSync() {
    ...
    console.log("🚀 表情分类：同步模块（包含无痕过滤版）已就绪");
    ...
    win.socket._onmessage = function(msg) {
      ...
      // 2. 拦截并过滤掉同步数据包，防止其在前端页面（如 yt.addPath）渲染显示
      if (typeof msg === 'string') {
        const syncPushRegex = /#~sync_push\|iiroseEmojiCategories\|(?:%7B.*?%7D)?~#/i;
        const syncReqRegex = /#~sync_req~#/i;
        
        if (syncPushRegex.test(msg) || syncReqRegex.test(msg)) {
          // IIROSE 公屏消息以 "" 开头，用 < 分隔
          if (msg.startsWith('""')) {
            const parts = msg.substring(2).split('<');
            const filteredParts = parts.filter(p => !syncPushRegex.test(p) && !syncReqRegex.test(p));
            
            // 如果这段消息全部都是同步指令，直接拦截阻止后续传递
            if (filteredParts.length === 0) return;
            
            // 否则重新拼接正常的消息
            msg = '""' + filteredParts.join('<');
          } else {
            // 私聊等其他类型指令，若包含特征则直接丢弃
            return;
          }
        }
      }
